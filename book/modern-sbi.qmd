...
In the next step we will replace this simple conditional affine flow with a **more expressive conditional flow** so the learned posterior can be non-Gaussian and multi-modal.
...
# Compare amortised vs local posterior mean estimates.
rows = []
for k, s_k in enumerate(s_obs_list):
    ...
    flow_loc = fit_local_flow(s_k, flow_init=flow, n_local=400, proposal_sd=0.15, rng=np.random.default_rng(10 + k))
...
print("k  beta_true  beta_hat(am)  |err|   beta_hat(local)  |err|")
for r in rows:
    print(
...
print({"mean_abs_err_amortised": mean_err_am, "mean_abs_err_local": mean_err_loc, "local_sims_per_obs": 400})
```

Interpretation (for this toy setup):

- The **amortised** model pays an up-front simulation cost (here: `len(S_train)` sims) but is then
  cheap to evaluate for each new observation.
- The **local** model spends simulations per observation (here: 400 per observation) and can adapt
  to a specific dataset by focusing the proposal region.
