...
#### Demo: beta-only “NPE” with a conditional Gaussian (no deep learning)
...
def fit_conditional_gaussian(
    S: np.ndarray, logb: np.ndarray
) -> dict[str, np.ndarray | float]:
    """Fit q(logbeta | s) = Normal(mu(s), sigma^2) with mu(s)=w^T z + b.

    We standardise features z = (s - mean) / std and solve least squares.
    """
    ...
    s_std = S.std(axis=0)
    s_std = np.where(s_std > 0, s_std, 1.0)
    Z = (S - s_mean) / s_std

    # Design matrix with intercept.
    X = np.column_stack([np.ones(len(Z)), Z])
    w_hat, *_ = np.linalg.lstsq(X, logb, rcond=None)
    ...
    return {"w": w_hat, "s_mean": s_mean, "s_std": s_std, "sigma": sigma_hat}


def predict_mu(model: dict[str, np.ndarray | float], s: np.ndarray) -> float:
    s = np.asarray(s, dtype=float)
    z = (s - model["s_mean"]) / model["s_std"]
    x = np.concatenate([[1.0], z])
    return float(x @ model["w"])


model = fit_conditional_gaussian(S_train, logb_train)

mu_test = np.array([predict_mu(model, s) for s in S_test])
rmse = float(np.sqrt(np.mean((mu_test - logb_test) ** 2)))

print({"sigma_hat": float(model["sigma"]), "test_RMSE_logbeta": rmse})
```
...
def sample_posterior_logbeta(
    ...
    *,
    ...
    mu = predict_mu(model, s_obs)
    sigma = float(model["sigma"])
    return rng.normal(loc=mu, scale=sigma, size=n)


logb_post = sample_posterior_logbeta(model, s_obs, 5_000, rng=np.random.default_rng(3))
...
In the next step we will replace this conditional Gaussian with a **conditional flow** so the learned posterior can be non-Gaussian and multi-modal.
