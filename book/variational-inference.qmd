...
- Explain the **ELBO** and how it relates to the KL divergence.
- Work through a **mean-field Gaussian VI** example end-to-end.
- Introduce **stochastic gradient VI** (SVI) and the **reparameterisation trick**.
- Connect VI to the rest of the handbook (classical baselines and modern SBI).

## Outline

1) VI in one diagram
2) The ELBO: what we optimise, and why
3) Choosing a variational family (mean-field vs richer families)
4) Optimisation strategies
5) Worked example: normal–normal model (NumPy)
...
try:
    ...
    print("Optional dependency missing; skipping JAX VI demo:", e)
```
