...
try:
    ...
    def simulate_seir_euler_jax(beta: float):
        ...
        (sT, eT, iT, rT), s_hist = jax.lax.scan(step_fn, init, xs=None, length=steps_j)

        # Prepend s0 so we have length steps+1 like the NumPy solver.
        s_all = jnp.concatenate([jnp.asarray([s0]), s_hist], axis=0)
        return s_all

    def nbinom_logpmf_jax(k: jax.Array, mu: jax.Array, dispersion: float) -> jax.Array:
        r = jnp.asarray(float(dispersion))
        p = r / (r + mu)
        logp = jnp.log(p)
        log1mp = jnp.log1p(-p)
        kf = k.astype(float)
    ...
    def log_post_logbeta_jax(position: jax.Array) -> jax.Array:
        logbeta = position[0]
